...
#ifdef APACHE20
static int test_init(apr_pool_t *p, server_rec *s)
#else
static void test_init(server_rec *s, apr_pool_t *p)
#endif
{
#ifdef WITH_OCI
...
static const command_rec tst_commands[] =
{
 {"test_val",
  ...
  (void *)XtOffsetOf(test_config, test_val),
  ACCESS_CONF, TAKE1, "test_val [value]"},
 {0}
};
