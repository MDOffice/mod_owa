...
void morq_create_mutex(request_rec *request, owa_context *octx)
{
    ...
    if (!(octx->multithread))
        return;
    if (octx->mtctx->c_mutex != os_nullmutex)
        return;
    /*
...
{
    if (zero_flag)
        return (apr_pcalloc(request->pool, sz));
...
int morq_regex(request_rec *request, char *pattern, char **str, int caseflag)
{
    ...
    if (!rules)
        return (0);
    if (ap_regexec(rules, *str, 1, &match, 0))
        return (0);
    *str += match.rm_so;
...
{
    if (range_flag)
        request->clength = (apr_off_t)len;
    else
        ap_set_content_length(request, (apr_off_t)len);
}
...
int morq_stream(request_rec *request, int flush_flag)
{
    int status;
    if (flush_flag)
        return (ap_discard_request_body(request));
    status = ap_setup_client_block(request, REQUEST_CHUNKED_DECHUNK);
    if (!status)
        status = (ap_should_client_block(request)) ? 0 : -1;
    return (status);
}
                    ...
                    char *name, char *value)
{
    ...
    {
    case OWA_TABLE_SUBPROC:
        tab = request->subprocess_env;
        break;
    case OWA_TABLE_HEADOUT:
        tab = request->headers_out;
        break;
    case OWA_TABLE_HEADERR:
        tab = request->err_headers_out;
        break;
    default:
        return;
        break;
    ...
    if (set_flag)
        apr_table_set(tab, name, value);
    else
        apr_table_add(tab, name, value);
}
...
static int get_dav_method(int m)
{
    ...
    /* DAV methods */
    case M_OPTIONS:
        x = DAV_METHOD_OPTIONS;
        break;
    case M_CONNECT:
        x = DAV_METHOD_CONNECT;
        break;
    case M_TRACE:
        x = DAV_METHOD_TRACE;
        break;
    case M_PROPFIND:
        x = DAV_METHOD_PROPFIND;
        break;
    case M_PROPPATCH:
        x = DAV_METHOD_PROPPATCH;
        break;
    case M_MKCOL:
        x = DAV_METHOD_MKCOL;
        break;
    ...
    case M_VERSION_CONTROL:
        x = DAV_METHOD_VERSION_CONTROL;
        break;
    case M_CHECKOUT:
        x = DAV_METHOD_CHECKOUT;
        break;
    case M_UNCHECKOUT:
        x = DAV_METHOD_UNCHECKOUT;
        break;
    case M_CHECKIN:
        x = DAV_METHOD_CHECKIN;
        break;
    case M_UPDATE:
        x = DAV_METHOD_UPDATE;
        break;
    ...
    case M_REPORT:
        ...
        break;
    case M_MKWORKSPACE:
        x = DAV_METHOD_MKWORKSPACE;
        break;
    case M_MKACTIVITY:
        x = DAV_METHOD_MKACTIVITY;
        break;
    case M_BASELINE_CONTROL:
        x = DAV_METHOD_BASELINE_CONTROL;
        break;
    case M_MERGE:
        x = DAV_METHOD_MERGE;
        break;
...
static int mowa_handler(request_rec *r)
{
    ...
    if (str_compare(r->handler, "owa_handler", -1, 1))
        return (DECLINED);
...
static const char *mowa_diag(cmd_parms *cmd, owa_context *octx, char *dstr)
{
    ...
    */
    ...
    {
        diagflag = 0;
        if (str_substr(dstr, "COMMAND", 1))
            diagflag |= DIAG_COMMAND;
        if (str_substr(dstr, "ARGS", 1))
            diagflag |= DIAG_ARGS;
        if (str_substr(dstr, "CGIENV", 1))
            diagflag |= DIAG_CGIENV;
        if (str_substr(dstr, "POOL", 1))
            diagflag |= DIAG_POOL;
        if (str_substr(dstr, "HEADER", 1))
            diagflag |= DIAG_HEADER;
        if (str_substr(dstr, "COOKIES", 1))
            diagflag |= DIAG_COOKIES;
        if (str_substr(dstr, "CONTENT", 1))
            diagflag |= DIAG_CONTENT;
        if (str_substr(dstr, "RESPONSE", 1))
            diagflag |= DIAG_RESPONSE;
        if (str_substr(dstr, "TIMING", 1))
            diagflag |= DIAG_TIMING;
        if (str_substr(dstr, "SQL", 1))
            diagflag |= DIAG_SQL;
        if (str_substr(dstr, "MEMORY", 1))
            diagflag |= DIAG_MEMORY;
        if (str_substr(dstr, "THREADS", 1))
            diagflag |= DIAG_THREADS;
        if (str_substr(dstr, "ERROR", 1))
            diagflag |= DIAG_ERROR;
    ...
    if (onls)
    {
        ...
        */
        ...
        if (i > 0)
        {
            ...
            if (!aptr)
                return (alloc_failure(cmd->server, i));
...
static const char *mowa_lobs(cmd_parms *cmd, owa_context *octx, char *ltypes)
{
    ...
    if (ltypes)
    {
        if (str_substr(ltypes, "BIN", 1))
            lobtypes = LOB_MODE_BIN;
        if (str_substr(ltypes, "CHAR", 1))
            lobtypes = LOB_MODE_CHAR;
        if (str_substr(ltypes, "NCHAR", 1))
            lobtypes = LOB_MODE_NCHAR;
        if (str_substr(ltypes, "FILE", 1))
            lobtypes = LOB_MODE_BFILE;
        if (lobtypes > octx->lobtypes)
            octx->lobtypes = lobtypes;
                              ...
                              char *logname, char *physname, char *life)
{
    ...
    */
    ...
    if ((n % CACHE_MAX_ALIASES) == 0)
    {
        ...
        if (!new_lifes)
            return (alloc_failure(cmd->server, n * sizeof(alias)));
...
static const char *mowa_http(cmd_parms *cmd, owa_context *octx, char *mode)
{
    if (mode)
    {
        if (str_compare(mode, "NORMAL", -1, 1) == 0)
            octx->dav_mode = 0;
        else if (str_compare(mode, "REST", -1, 1) == 0)
            octx->dav_mode = 1;
        else if (str_compare(mode, "DAV", -1, 1) == 0)
            octx->dav_mode = 2;
    }
    return ((char *)0);
}
...
static void *oracle_location_config(apr_pool_t *p, char *dirspec)
{
    ...
    */
    ...
    if (octx)
    {
        ...
        if (dirspec)
            str_copy(octx->location, dirspec);
...
#define ARG_PATTERN(nm, st, cf, tk, hlp) \
    {                                    \
        nm, st, cf, tk, hlp              \
    }
